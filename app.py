...
def ocr_base64_images_in_markdown(markdown_text, task_id=None):
    ...
    for idx, match in enumerate(matches):
        ...
        base64_data = match.group(3).replace('\r', '').replace('\n', '').replace(' ', '')
    ...
    try:
        ...
        # 3. OTHER DOCUMENTS (PDF, Word, Excel, etc.)
            ...
            if ext == 'pdf':
                ...
                try:
                    markdown_content = ocr_base64_images_in_markdown(markdown_content, task_id)
...
def sort_ocr_results_smart(results):
    ...
    for item in results:
        try:
            ...
            height = bottom - top
            items.append({'left': left, 'top': top, 'right': right, 'bottom': bottom,
    ...
    for item in items:
        ...
        for line in lines:
            ...
            if overlap_bottom > overlap_top:  # có overlap dọc
                overlap_ratio = (overlap_bottom - overlap_top) / max(item['height'], line_height, 1)
...
def postprocess_ocr_to_markdown(lines):
    ...
    for line in cleaned_lines:
        if merged_lines and (line[0].islower() or (line[0] in '.,)?:' and not line.startswith('...'))):
    ...
    for line in merged_lines:
        ...
        if re.match(r'^(Đúng|Sai|True|False|Chọn một\.|Chọn một|Chọn hai\.|Chọn hai|Điền vào chỗ trống\.?)$', line, re.IGNORECASE):
            md_lines.append(f'\n*{line}*\n')
            continue
...
def convert_url():
    ...
    if match:
        ...
        print(f"Da chuan hoa URL YouTube thanh: {url}. Dang lay transcript qua yt-dlp...")
        try:
