        ...
        if IS_YOUTUBE_TRANSCRIPT_CAPABLE:
            ...
            if "v" in params and params["v"][0]:
                ...
                try:
                    ...
                    # Bắt tất cả lỗi bao gồm cả VideoUnavailable do bị YouTube chặn IP/bot
                    print(f"Error fetching transcript for video {video_id}: {e}")
                    transcript_text = f"*(Không thể tự động trích xuất phụ đề từ YouTube do chính sách chặn bot hoặc video không có phụ đề công khai. Lỗi chi tiết: {str(e)})*"
            if transcript_text:
                webpage_text += f"\n### Transcript\n{transcript_text}\n"
